...
func populateWorktree(t *testing.T, fsys billy.Filesystem) {
	t.Helper()
	err := fsys.MkdirAll("modules/category/test-module1/test-flavour1", fs.ModePerm)
	assert.NoError(t, err)
	err = fsys.MkdirAll("modules/category/test-module1/test-flavour2", fs.ModePerm)
	assert.NoError(t, err)
	err = fsys.MkdirAll("modules/category/test-module2/test-flavour1", fs.ModePerm)
	assert.NoError(t, err)
	err = fsys.MkdirAll("modules/category/test-module3/test-flavour1", fs.ModePerm)
	assert.NoError(t, err)
	err = fsys.MkdirAll("add-ons/category/test-addon1/subdir", fs.ModePerm)
	assert.NoError(t, err)
	err = fsys.MkdirAll("add-ons/category/test-addon2/", fs.ModePerm)
	assert.NoError(t, err)
	err = fsys.MkdirAll("otherdir", fs.ModePerm)
	assert.NoError(t, err)
	_, err = fsys.Create("README.md")
	assert.NoError(t, err)
	f, err := fsys.Create("modules/category/test-module1/test-flavour1/file1.yaml")
	assert.NoError(t, err)
	_, err = f.Write([]byte("file1-1-1 content\n"))
	assert.NoError(t, err)
	err = f.Close()
	assert.NoError(t, err)
	f, err = fsys.Create("modules/category/test-module1/test-flavour1/file2.yaml")
	assert.NoError(t, err)
	_, err = f.Write([]byte("file1-1-2 content\n"))
	assert.NoError(t, err)
	err = f.Close()
	assert.NoError(t, err)
	f, err = fsys.Create("modules/category/test-module1/test-flavour2/file1.yaml")
	assert.NoError(t, err)
	_, err = f.Write([]byte("file1-2-1 content\n"))
	assert.NoError(t, err)
	err = f.Close()
	assert.NoError(t, err)
	_, err = fsys.Create("modules/category/test-module2/test-flavour1/file1.yaml")
	assert.NoError(t, err)
	_, err = fsys.Create("add-ons/category/test-addon1/file1.yaml")
	assert.NoError(t, err)
	_, err = fsys.Create("add-ons/category/test-addon1/subdir/file1.yaml")
	assert.NoError(t, err)
	_, err = fsys.Create("add-ons/category/test-addon2/file1.yaml")
	assert.NoError(t, err)
}
